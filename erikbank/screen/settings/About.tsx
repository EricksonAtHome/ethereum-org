...
const About: React.FC = () => {
  ...
  const handleOnXPress = useCallback(() => {
    presentAlert({ message: 'Follow ErikBank on social media — links coming soon.' });
  }, []);

  const handleOnTelegramPress = useCallback(() => {
    presentAlert({ message: 'Join the ErikBank community — links coming soon.' });
  }, []);
  ...
  const handleOnRatePress = useCallback(async () => {
    // ErikBank: configure your App Store / Play Store URLs when published
    presentAlert({ message: 'ErikBank is not yet published on app stores.' });
  }, []);
